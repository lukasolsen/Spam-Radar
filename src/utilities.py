...
class Api(object):
    ...
    def pullout (self, m, key):
        ...
        if not m.is_multipart():
            if m.get_filename(): # It's an attachment
                fn = m.get_filename()
                cfn = self.construct_name(key, fn)
                Files[fn] = (cfn, None)
                if self.file_exists(cfn): return Text, Html, Files, 1
                self.save_file(cfn, m.get_payload(decode=True))
                return Text, Html, Files, 1
            # Not an attachment!
            # See where this belongs. Text, Html or some other data:
            cp = m.get_content_type()
            if str(cp)=="text/plain": Text += m.get_payload()
            elif str(cp)=="text/html": Html += m.get_payload()
            else:
                # Something else!
    ...
    def extract (self, msgfile, key):
        ...
        From, To, Subject, Date = self.caption(m)
        Text, Html, Files, Parts = self.pullout(m, key)
    ...
    def caption (self, origin):
            ...
            """
            Date = ""
            if str(origin).__contains__("date"): Date = origin["date"].strip()
            From = ""
            if str(origin).__contains__("from"): From = origin["from"].strip()
            To = ""
            if str(origin).__contains__("to"): To = origin["to"].strip()
            Subject = ""
            if str(origin).__contains__("subject"): Subject = origin["subject"].strip()
